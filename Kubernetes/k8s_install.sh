...
# Supports Kubernetes v1.34.2, containerd, flannel, kubelet swapoff fix,
...
configure_firewalld(){
 ...
 if systemctl is-enabled firewalld >/dev/null 2>&1; then
   firewall-cmd --add-port=61050/tcp --permanent || true
...
kubeadm_init_cp(){
 write_kubeadm_init_config
 kubeadm init --config /root/kubeadm/kubeadm-config.yaml --upload-certs
 mkdir -p $HOME/.kube
